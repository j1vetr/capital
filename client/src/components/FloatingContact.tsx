...
export function FloatingContact() {
  ...
  return (
    /* Outer anchor: fixed position, wraps only the toggle button.
       The contact panel is positioned absolutely above the button
       so it never occupies flex/block space and does not shift the button. */
    <div className="fixed bottom-6 right-6 z-50">
      {/* Contact panel — absolute, anchored to the bottom of this wrapper,
          always in the DOM for CSS transitions, hidden from a11y when closed */}
      <div
        /* inert removes all focusability and interactivity from descendants */
        inert={!isOpen || undefined}
        aria-hidden={!isOpen}
        style={{
          position: "absolute",
          bottom: "calc(100% + 1rem)", // 1rem = gap above toggle
          right: 0,
          opacity: isOpen ? 1 : 0,
          transform: isOpen
